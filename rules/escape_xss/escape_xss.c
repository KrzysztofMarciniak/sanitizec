 ...
 * @param input The dynamically allocated string to be escaped (ownership transferred and freed here).
...
char* escape_xss_apply(char* input, char **errmsg) {
    ...
    if (!input) {
        if (errmsg) *errmsg = strdup_safe("XSS Escape Rule: Input string is NULL.");
        return NULL;
    }
    ...
    if (!new_string) {
        free(input); // Free the original string since we failed to allocate the new one
        if (errmsg) *errmsg = strdup_safe("XSS Escape Rule: Memory allocation failed for output string.");
        return NULL;
    }
