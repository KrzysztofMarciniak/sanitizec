...
// 1. Function signature for all rule apply functions (char* rule_apply(char* input, char **errmsg))
typedef char* (*rule_func_t)(char*, char**);

// 2. Structure to map a rule flag (bitmask) to its implementation function and name
typedef struct {
    int flag;
    rule_func_t func;
...
// 3. The central lookup table mapping flags to functions.
...
/**
 * @brief Applies a ruleset of sanitization techniques to an input string synchronously.
 *
 ...
 * @return char* A new, dynamically allocated sanitized string, or NULL on error.
 */
char* sanitizec_apply(const char* input, int ruleset, char **errmsg) {
    ...
    if (!current_string) {
        if (errmsg) *errmsg = strdup_safe("Failed to allocate initial copy of input string.");
        return NULL;
    }
    ...
    for (int i = 0; rules_map[i].flag != 0; i++) {
        if (ruleset & rules_map[i].flag) {
            ...
            // Call the function associated with the flag, passing the current string and the error pointer
            char* new_string = rules_map[i].func(current_string, errmsg);

            if (new_string == NULL) {
                // Failure: The rule function should have already set *errmsg on failure.
                // We provide a fallback message if it failed to set one.
                if (errmsg && *errmsg == NULL) {
                    size_t msg_len = strlen(rules_map[i].name) + strlen(" failed during execution.") + 1;
                    char *default_msg = malloc(msg_len);
                    if (default_msg) {
                        // Use snprintf for safe string concatenation
                        snprintf(default_msg, msg_len, "%s failed during execution.", rules_map[i].name);
                        *errmsg = default_msg;
                    }
                }
                // The rule function is responsible for freeing `current_string` on failure.
                return NULL;
            }
