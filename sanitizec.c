...
static const rule_map_t rules_map[] = {
    ...
    {SANITIZEC_RULE_ALPHANUMERIC_ONLY, alphanumeric_only_apply, "Letters and Numbers only"},
...
char* sanitizec_apply(const char* input, int ruleset, char** errmsg) {
        ...
        if (!current_string) {
                if (errmsg)
                        *errmsg = strdup_safe("Failed to allocate initial copy of input string.");
                return NULL;
        }
        ...
        for (int i = 0; rules_map[i].flag != 0; i++) {
                if (ruleset & rules_map[i].flag) {
                        // Call the function associated with the flag, passing the current string
                        // and the error pointer
                        char* new_string = rules_map[i].func(current_string, errmsg);

                        if (new_string == NULL) {
                                // Failure: The rule function should have already set *errmsg on
                                // failure. We provide a fallback message if it failed to set one.
                                if (errmsg && *errmsg == NULL) {
                                        size_t msg_len = strlen(rules_map[i].name) +
                                                         strlen(" failed during execution.") + 1;
                                ...
                                // The rule function is responsible for freeing `current_string` on
                                // failure.
                                return NULL;
                        }
